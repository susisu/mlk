...
// a -> String
prelude["typeOf"] = new Value(DataType.FUNCTION, function (_args) {
    if (_args.length !== 1) { throw wrongNumberOfArgumentsError(undefined, 1, _args.length); }
    return new Value(DataType.STRING, _args[0].type);
});
// a -> a
...
// a -> Reference
prelude["ref"] = new Value(DataType.FUNCTION, function (_args) {
    if (_args.length !== 1) { throw wrongNumberOfArgumentsError(undefined, 1, _args.length); }
    return new Value(DataType.REFERENCE, _args[0]);
});
// Number -> Array
prelude["array"] = new Value(DataType.FUNCTION, function (_args) {
    if (_args.length !== 1) { throw wrongNumberOfArgumentsError(undefined, 1, _args.length); }
    if (_args[0].type !== DataType.NUMBER) { throw typeError(undefined, DataType.NUMBER, _args[0].type); }
...
prelude["range"] = new Value(DataType.FUNCTION, function (_args) {
    var step = 1;
    switch(_args.length) {
        case 2:
            if (_args[0].type !== DataType.NUMBER) { throw typeError(undefined, DataType.NUMBER, _args[0].type); }
    ...
    var arr = [];
    for (var i = _args[0].data; i <= _args[1].data; i += step) {
        arr.push(new Value(DataType.NUMBER, i));
    }
    return new Value(DataType.ARRAY, arr);
});
...
prelude["~_"] = new Value(DataType.FUNCTION, function (_args) {
    if (_args.length !== 1) { throw wrongNumberOfArgumentsError(undefined, 1, _args.length); }
    if (_args[0].type !== DataType.REFERENCE) { throw typeError(undefined, DataType.REFERENCE, _args[0].type); }
    return _args[0].data;
});
// Array -> Number -> a
