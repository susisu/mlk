...
function initParser() {
    ...
    var letInBinding = tokenParser.identifier.bind(function (name) {
        return tokenParser.reservedOp("=")
            .then(expression)
            .bind(function (bindExpr) {
                return lq.pure([name, bindExpr]);
            });
    });
    var letIn = lq.getPosition.bind(function (pos) {
        return tokenParser.reserved("let")
            .then(tokenParser.commaSep1(letInBinding))
                ...
                return tokenParser.reserved("in")
                    ...
                    .bind(function (expr) {
                        ...
                        for (var i = 0; i < bindings.length; i++) {
                            names.push(bindings[i][0]);
                            bindExprs.push(bindings[i][1]);
                        }
                        return lq.pure(new Application(pos, new Lambda(pos, names, expr), bindExprs));
                    });
            });
    }).label("let in");
